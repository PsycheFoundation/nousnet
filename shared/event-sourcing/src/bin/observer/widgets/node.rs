...
impl<'a> Widget for NodeWidget<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let node_entry = self.snapshot.nodes.get_index(self.selected_node_idx);

        let title = match node_entry {
            Some((id, _)) => {
        ...
        let Some((_, node)) = node_entry else {
            Paragraph::new("No nodes").render(inner, buf);
            return;
        };
